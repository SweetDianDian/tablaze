...
引用将快照版本与观察到的节点、语义元数据关联，等待后再次检查，但无法消除检查到输入之间的竞态。预检可能滚动并触发逻辑。默认工具集没有通用 eval；显式 `--page-script` 会增加拥有页面登录状态和网络权限的脚本工具，不能与密钥配置、外部 CDP 或导航策略同时使用。取消和超时关闭自有会话、停止后续操作，但不撤回已经生效的提交或外部请求。失败步骤也可能有副作用，重试前应检查部分完成字段与业务结果。详见[页面脚本权限与恢复边界](docs/PAGE_SCRIPT.md)。
