// Explicit opt-in real-model smoke for checked partial output; not a competitor comparison.
...
const schemaDirectory = await mkdtemp(join(tmpdir(), 'tablaze-partial-output-smoke-'));
const schemaPath = join(schemaDirectory, 'schema.json');
await writeFile(schemaPath, JSON.stringify(schema) + '\n', { mode: 0o600 });
async function sourceHash() {
  const paths = ['package.json', 'package-lock.json', 'tsconfig.json', 'bench/codex-partial-output-smoke.mjs', 'bench/comparison/fixture.mjs'];
...
const report = {
  kind: 'production-codex-partial-output-live-smoke', schema_version: 1,
  started_at: new Date().toISOString(), comparison: false,
  purpose: 'One real Codex task with a checked partial on an incomplete run and independent server-side acceptance.',
  ...
  partial_schema: schema, source_sha256: await sourceHash(),
...
try {
  ...
  const args = ['dist/cli.js', 'run', '--provider', 'codex', '--codex-command', values['codex-command'], '--model', values.model,
    ...
    '--task', `${attempt.prompt}\nAfter submitting, use tab_verify to confirm the actual accepted result. Publish one checked partial under key receipt-total with JSON data {"total": <accepted total>} and cite the passing tab_verify call ID. Then request human input for which separate record to process next. Do not mark the whole task complete.`,
    '--start-url', attempt.url, '--partial-schema', schemaPath, '--channel', channel,
  ...
  const published = execution.cli?.partials?.find(partial => partial.key === 'receipt-total');
  const partialMatchesServer = judge.passed && published?.data?.total === judge.evidence.records[0]?.total;
  const evidencePresent = Boolean(published?.evidence?.some(item => item.toolCallId && item.sessionId && item.checks?.length > 0 && item.checks.every(check => check.pass === true)));
  report.attempt = { task_id: 'extraction', task_sha256: attempt.taskHash, prompt_sha256: hash(args[args.indexOf('--task') + 1]), ...execution, independent_judge: judge, partial_matches_server: partialMatchesServer, partial_evidence_present: evidencePresent,
    complete_success: execution.exit_code === 2 && execution.cli?.status === 'needs_input' && partialMatchesServer && evidencePresent };
  console.log(JSON.stringify({ status: execution.cli?.status ?? 'no_report', accepted: judge.passed, partial_matches_server: partialMatchesServer, partial_evidence_present: evidencePresent, whole_run_ms: execution.whole_run_ms }));
} finally {
  await service.close();
