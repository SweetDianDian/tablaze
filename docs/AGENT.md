...
`--start-url` optionally opens one explicitly supplied HTTP(S) URL before the first model decision. The opt-in `--direct-open-task-url` instead selects a single unambiguous HTTP(S) URL from the trusted task text; the SDK equivalent is `directOpenTaskUrl: true`. Multiple URLs, malformed URLs and embedded credentials leave navigation to the planner. An explicit `startUrl` wins when both are supplied. Neither mode takes a URL from a page, tool result or model output; task prose is never scanned by default. Enable task extraction only when its sole URL is intended as the starting page.
...
For trusted setup before model planning, use `initialActions` in the SDK or `--initial-actions ./initial-actions.json` in the CLI. The file is a regular UTF-8 JSON array, at most 64 KiB, with 1–20 actions. The first entry must navigate; later entries may navigate or click a uniquely named control:

```json
[
  {"url":"https://example.com/start"},
  {"click":{"name":"Open details","role":"button"}},
  {"url":"https://example.com/second","newTab":true}
]
```

The first entry opens one owned session. Later URLs with `newTab: true` open a tab in that session; otherwise they navigate its active tab. Every URL must be absolute HTTP(S) without embedded credentials. A click uses an exact accessible name in the active tab's main document and may specify `role` (`button`, `link`, `menuitem`, `tab`, `checkbox`, or `radio`). The tool takes a fresh bounded observation, rejects missing, ambiguous or truncated targets, and rechecks the chosen element before input. It performs a real browser click, so it can trigger navigation or a write; only trusted callers should configure it. This sequence is mutually exclusive with `--start-url` and `--direct-open-task-url`. It runs before the first model request, using the same MCP tools and call budget, and every attempted action is persisted separately. A failed action stops the sequence and leaves its result for the planner. An unknown outcome requires trusted reconciliation; the action is never automatically replayed. A resume after a confirmed successful action continues only the remaining entries in the restored session. Version-six navigation checkpoints migrate to version seven. The current pre-model format still does not cover arbitrary Browser Use actions, text input or frame-specific clicks.
...
try {
  const result = await runAgent({
    ...
    startUrl: "https://example.com", // Optional explicit initialization.
