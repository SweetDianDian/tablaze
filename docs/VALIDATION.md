...
The normal recording completed on 2026-09-22 at 15:58:38 UTC with `status: passed` and `quick_mode: false`, using Node.js 25.9.0 and Chrome 153.0.8010.53 on macOS arm64. It is 126 seconds at 1440×900, including 121 seconds of labelled reading holds. [Recorder and method](../demo/README.md) · [Complete request evidence](evidence/demo-run.json).
...
The report records 22 MCP tool calls, eight passing browser assertions and 2,339.659 ms in summed tool-call durations. Those durations exclude presentation holds and model inference. This single local scripted run is not an internet performance benchmark or a Browser Use comparison.

本次正式录制通过验收：同一条差旅流程完成六步批次、五项结果检查、旧引用拒绝且无审批写入、重新观察恢复、弹窗内四步审批、三项回执检查，以及 CSV 实际字节和哈希核对。最终一份订单、一个已核对文件、两个自有标签页关闭，剩余会话为零。视频长 126 秒，含 121 秒讲解停留；22 次 MCP 调用的耗时合计仅描述本地样本。
...
All 30 local Chrome checks passed at 390, 768 and 1440 pixels in both languages, with zero page errors or horizontal overflow. All seven chapters sought to their reported times and updated the active state; language switching retained progress. Overlay playback, native pause, Enter/Space controls, reduced-motion behavior and no autoplay passed. Missing-report playback, disabled-demo loading and failed-video download fallback also passed. A complete accelerated decode check reached the end of the unchanged 126-second media without a video error; playback rate was reset to one. Two frames decoded from the compact 5 fps video were visually checked for readable approval fields and file evidence. [Website QA evidence](evidence/demo-site-qa-v2.json).
