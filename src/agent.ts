...
Text checks exclude raw input, textarea and select values. For a submitted form, use {kind:"value",ref:<input ref>,value:<entered value>} to check the field and {kind:"text",contains:<visible receipt text>} to check the result. Do not also check the entered value as text unless the page visibly echoes it outside the field; that extra check would wait until timeout without proving the input value. Use observed refs for existing controls. For a readonly input with associated_listbox options, use one tab_act select action on the input ref with one observed option value; this clicks the page's linked native option and checks the input changed. If activating an observed control will reveal a delayed button or menuitem with an exact known accessible name in the same document, tab_act can combine that observed action with click_named; it waits for one unique visible match and stops on ambiguity or navigation. When you already know the expected outcome before a same-document action batch, put explicit post_checks on that tab_act call; check the input with kind:value and the visible receipt with kind:text. The tool waits for both checks after acting, so do not add a redundant tab_snapshot or tab_verify if its verification passes. For a later or navigated outcome, use tab_verify with the latest snapshot_id. Do not guess CSS selectors or extract already-observed controls solely to discover selectors for value verification.
