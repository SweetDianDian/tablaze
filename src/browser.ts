      ...
      const launch = this.options.cdpUrl
        ...
        : this.options.profileDir
          ? acquireOwnedProfile(this.options.profileDir, this.options.expectedProfileId).then(async lease => {
            ...
            try {
              const context = await chromium.launchPersistentContext(lease.directory, { headless: this.options.headless ?? true, channel: this.options.channel, executablePath: this.options.executablePath, timeout: 30000, viewport: this.options.viewport ?? { width: 1280, height: 800 }, screen: this.options.screen, deviceScaleFactor: this.options.deviceScaleFactor, userAgent: this.options.userAgent, locale: this.options.locale, timezoneId: this.options.timezoneId, isMobile: this.options.isMobile, hasTouch: this.options.hasTouch, permissions: this.options.permissions, proxy: this.options.proxy, acceptDownloads: true });
