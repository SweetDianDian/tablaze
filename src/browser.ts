...
export class BrowserEngine {
  ...
  constructor(private options: BrowserOptions = {}) {
    if (options.devicePreset !== undefined) {
      ...
      if (options.viewport || options.noViewport || options.screen || options.deviceScaleFactor !== undefined || options.userAgent !== undefined || options.isMobile !== undefined || options.hasTouch !== undefined) throw new BrowserError('INVALID_ARGUMENT', 'A device preset cannot be combined with manual device emulation settings.');
    ...
    if (options.noViewport !== undefined && typeof options.noViewport !== 'boolean') throw new BrowserError('INVALID_ARGUMENT', 'noViewport must be a boolean.');
    if (options.noViewport && (options.viewport || options.devicePreset || options.screen || options.isMobile)) throw new BrowserError('INVALID_ARGUMENT', 'noViewport cannot be combined with fixed viewport or mobile/screen emulation.');
    if (options.windowSize && (typeof options.windowSize !== 'object' || options.windowSize === null || Array.isArray(options.windowSize) || !Number.isInteger(options.windowSize.width) || !Number.isInteger(options.windowSize.height) || options.windowSize.width < 320 || options.windowSize.width > 3840 || options.windowSize.height < 240 || options.windowSize.height > 2160)) throw new BrowserError('INVALID_ARGUMENT', 'windowSize width must be 320–3840 and height 240–2160.');
    if (options.windowPosition && (typeof options.windowPosition !== 'object' || options.windowPosition === null || Array.isArray(options.windowPosition) || !Number.isInteger(options.windowPosition.x) || !Number.isInteger(options.windowPosition.y) || Math.abs(options.windowPosition.x) > 10000 || Math.abs(options.windowPosition.y) > 10000)) throw new BrowserError('INVALID_ARGUMENT', 'windowPosition x and y must be integers from -10000 to 10000.');
    ...
    if (options.userAgent !== undefined && (typeof options.userAgent !== 'string' || !options.userAgent.trim() || options.userAgent.length > 1024 || /[\u0000-\u001f\u007f]/.test(options.userAgent))) throw new BrowserError('INVALID_ARGUMENT', 'userAgent must be a nonempty printable string of at most 1024 characters.');
    if (options.locale !== undefined) { try { if (typeof options.locale !== 'string' || options.locale.length > 80 || !options.locale.trim()) throw new Error(); new Intl.Locale(options.locale); } catch { throw new BrowserError('INVALID_ARGUMENT', 'locale must be a valid BCP 47 language tag.'); } }
    if (options.timezoneId !== undefined) { try { if (typeof options.timezoneId !== 'string' || options.timezoneId.length > 100 || !options.timezoneId.trim()) throw new Error(); new Intl.DateTimeFormat('en-US', { timeZone: options.timezoneId }); } catch { throw new BrowserError('INVALID_ARGUMENT', 'timezoneId must be a supported IANA time zone.'); } }
    ...
    if (options.cdpUrl && (options.devicePreset !== undefined || options.viewport || options.noViewport || options.windowSize || options.windowPosition || options.screen || options.deviceScaleFactor !== undefined || options.userAgent !== undefined || options.locale !== undefined || options.timezoneId !== undefined || options.isMobile !== undefined || options.hasTouch !== undefined || options.permissions !== undefined)) throw new BrowserError('BROWSER_CONFIG_CDP_UNSUPPORTED', 'Browser emulation and permissions require a browser context owned by this engine.');
    if ((options.windowSize || options.windowPosition) && options.headless !== false) throw new BrowserError('INVALID_ARGUMENT', 'Browser window size and position require headed mode.');
  ...
  private windowArguments(): string[] {
    return [
      ...(this.options.windowSize ? [`--window-size=${this.options.windowSize.width},${this.options.windowSize.height}`] : []),
      ...(this.options.windowPosition ? [`--window-position=${this.options.windowPosition.x},${this.options.windowPosition.y}`] : []),
    ];
  }
      ...
      const launch = this.options.cdpUrl
        ...
        : this.options.profileDir
          ? acquireOwnedProfile(this.options.profileDir, this.options.expectedProfileId).then(async lease => {
            ...
            try {
              const context = await chromium.launchPersistentContext(lease.directory, { headless: this.options.headless ?? true, channel: this.options.channel, executablePath: this.options.executablePath, timeout: 30000, args: this.windowArguments(), viewport: this.options.noViewport ? null : this.options.viewport ?? { width: 1280, height: 800 }, screen: this.options.screen, deviceScaleFactor: this.options.deviceScaleFactor, userAgent: this.options.userAgent, locale: this.options.locale, timezoneId: this.options.timezoneId, isMobile: this.options.isMobile, hasTouch: this.options.hasTouch, permissions: this.options.permissions, proxy: this.options.proxy, acceptDownloads: true });
        ...
        : chromium.launch({ headless: this.options.headless ?? true, channel: this.options.channel, executablePath: this.options.executablePath, timeout: 30000, args: [...this.windowArguments(), ...(this.navigationPolicy ? ['--remote-debugging-port=0', '--enable-automation'] : [])] });
  ...
  act(sessionId: string, snapshotId: string, actions: BrowserAction[], options: { snapshot?: boolean; signal?: AbortSignal; timeoutMs?: number } = {}): Promise<Record<string, unknown>> {
    return this.exclusive(sessionId, async session => {
      ...
      const output: Record<string, unknown> = { ok: !failed && !interruption, batch_complete: !failed && !interruption && completed === actions.length, session_id: session.id, snapshot_id: snapshotId, action_frame_id: session.frames.get(state.frame) ?? null, partial: !!(failed || interruption) && (completed > 0 || failedActionMayHaveSideEffects) || !!popupFollowed && completed < actions.length, completed, failed, failed_action_may_have_side_effects: failedActionMayHaveSideEffects, results, ...(popupFollowed ? { replan_required: true, popup_followed: popupFollowed } : {}), elapsed_ms: Math.round(performance.now() - start) };
