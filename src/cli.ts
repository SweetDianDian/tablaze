...
function parseOptions(): { command: string; options: BrowserOptions; run?: RunOptions } {
  ...
  if (devicePreset !== undefined && (values.viewport !== undefined || values.screen !== undefined || values["device-scale-factor"] !== undefined || values["user-agent"] !== undefined || values.mobile || values.touch)) throw new Error("--device-preset cannot be combined with manual device emulation flags.");
  if (cdpUrl && (devicePreset !== undefined || values.viewport !== undefined || values.screen !== undefined || values["device-scale-factor"] !== undefined || values["user-agent"] !== undefined || values.locale !== undefined || values.timezone !== undefined || values.mobile || values.touch || values.permissions !== undefined)) throw new Error("Browser emulation and permissions require an owned browser context, not --cdp-url.");
  ...
  return { command: positionals[0] ?? "stdio", options: { headless: !values.headed, channel, executablePath: executablePath ? resolve(executablePath) : undefined, cdpUrl, profileDir: values["profile-dir"] !== undefined ? resolve(values["profile-dir"]) : undefined, expectedProfileId: values["profile-id"], devicePreset, viewport, screen, deviceScaleFactor, userAgent: values["user-agent"], locale: values.locale, timezoneId: values.timezone, isMobile: values.mobile, hasTouch: values.touch, permissions, proxy, timeoutMs, popupPolicy, navigationPolicy, secrets, captureNetwork: values["capture-network"] ?? false, recordVideo: values["record-video"] ?? false, recordHar: values["record-har"] ?? false, recordTrace: values["record-trace"] ?? false, allowPageScript: values["page-script"] ?? false }, run };
}
...
function doctor(options: BrowserOptions): void {
  ...
  const output = {
    ...
    headless: options.cdpUrl ? null : options.headless, timeout_ms: options.timeoutMs, popup_policy: options.popupPolicy ?? "stay", record_video: options.recordVideo ?? false, record_har: options.recordHar ?? false, record_trace: options.recordTrace ?? false,
    device_preset: options.devicePreset ?? null, viewport: options.viewport ?? preset?.viewport ?? { width: 1280, height: 800 }, screen: options.screen ?? preset?.screen ?? null, device_scale_factor: options.deviceScaleFactor ?? preset?.deviceScaleFactor ?? 1, user_agent_configured: options.userAgent !== undefined || preset !== undefined, locale: options.locale ?? null, timezone: options.timezoneId ?? null, mobile: options.isMobile ?? preset?.isMobile ?? false, touch: options.hasTouch ?? preset?.hasTouch ?? false, permissions: options.permissions ?? [],
