...
function parseOptions(): { command: string; options: BrowserOptions; run?: RunOptions } {
  ...
  if (cdpUrl && (values.viewport !== undefined || values.screen !== undefined || values["device-scale-factor"] !== undefined || values["user-agent"] !== undefined || values.locale !== undefined || values.timezone !== undefined || values.mobile || values.touch || values.permissions !== undefined)) throw new Error("Browser emulation and permissions require an owned browser context, not --cdp-url.");
...
function doctor(options: BrowserOptions): void {
  ...
  const output = {
    ...
    viewport: options.viewport ?? { width: 1280, height: 800 }, screen: options.screen ?? null, device_scale_factor: options.deviceScaleFactor ?? 1, user_agent_configured: options.userAgent !== undefined, locale: options.locale ?? null, timezone: options.timezoneId ?? null, mobile: options.isMobile ?? false, touch: options.hasTouch ?? false, permissions: options.permissions ?? [],
