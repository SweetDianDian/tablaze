...
const actions = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("upload"), ref, files: z.array(z.string().min(1).max(4096)).max(20).describe("Use an id from snapshot.available_files, or an explicitly permitted absolute path. Empty list clears selection.") }).strict(),
  z.object({ type: z.literal("upload_chooser"), ref, files: z.array(z.string().min(1).max(4096)).max(20).describe("Use an id from snapshot.available_files, or an explicitly permitted absolute path.") }).strict(),
...
const checks = z.union([
  ...
  z.object({ kind: z.literal("text"), contains: z.string().min(1).max(2_000).describe("Visible page text outside input, textarea and select values. For submitted field data, use a value check; use text for a visible receipt or status.") }).strict().describe("Check rendered page text, never the raw value inside a form control."),
  z.object({ kind: z.literal("visible"), selector }).strict(),
  z.object({ kind: z.literal("value"), selector, value: z.string().max(10_000).describe("Exact live input, textarea or select value; use this to confirm entered field data.") }).strict(),
  z.object({ kind: z.literal("value"), ref, value: z.string().max(10_000).describe("Exact live input, textarea or select value; use this to confirm entered field data.") }).strict(),
...
export function createServer(options: BrowserOptions = {}): { server: McpServer; engine: BrowserEngine; dispose: () => Promise<void> } {
  ...
  server.registerTool("tab_act", {
    title: "Act on observed elements", description: "Execute 1–20 ordered actions against a current snapshot. A select action on a readonly input with associated_listbox options opens its explicitly linked native listbox and clicks one option value, preserving the page's click callback; ordinary select refs use native selection. After an observed activating action in this batch, click_named can wait for one newly appearing visible button or menuitem by exact accessible name in the same document; ambiguity, replacement and navigation stop input. Use refs for controls already observed. Optional post_checks run only after the whole batch completes, using this same snapshot's refs; they poll for asynchronous outcomes and return verification evidence in this call. Check entered field data with kind:value and a visible save receipt with kind:text; do not also check the entered value as text unless the page visibly echoes it outside the field. A failed postcondition keeps completed effects, sets replan_required, and skips later queued tools. Ref checks fail if the document or node changes; use separate tab_verify after navigation. Use fill_secret with an observed ref and a secret alias from snapshot.available_secrets for configured credentials. For uploads, use an id from snapshot.available_files; Agent runs allow only trusted files and completed session downloads. Includes hover, double_click, explicit local-file upload, and click_xy in main-viewport CSS pixels after visual inspection (coordinates lack DOM identity guards). Default 30s action budget, maximum 60s; verification has a separate timeout. Stops on first action failure. A followed popup returns its snapshot and replan_required without running post_checks. Cancellation closes the session; completed effects remain.",
